...
export default {
  ...
  methods: {
    ...
    async initchartsdata () {
      ...
      if (res.code === 200) {
        this.initchars((res.data.columnList))
    ...
    initchars (data) {
      ...
      data.forEach(item => {
        ...
        x.push(setTime({ date: item.countTime, type: 3, number: 1000 }))
        let obj = {
          data: item.sumTotal,
          time: setTime({ date: item.countTime, type: 3, number: 1000 })
        }
      ...
      this.oneCharts.setOption({
        color: ['#ffc333'],
        title: {
          text: `${this.townName}${this.villageName}每月用水量`,
          x: 'left',
          y: 'top',
          textStyle: {
            color: '#385276',
