    ...
    <div class="box">
      <div class="boxContent">
        <el-table :data="tableData"
          ...
          <el-table-column prop="camerCode"
            label="监控编号"
            align="center"></el-table-column>
      ...
      <div v-if="this.state === '1' || this.state === '2'">
        <el-form :model="ruleForm"
          ...
          <el-row type="flex"
            ...
            <el-col :span='11'>
              <el-form-item label="监控编号:" prop="camerCode">
                <el-input v-model="ruleForm.camerCode"></el-input>
                <!-- <el-select v-model="ruleForm.stationId"
                  placeholder="请选择">
                  <el-option v-for="item in stationLists"
                    :key="item.name"
                    :label="item.name"
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        cameraName: [{ required: true, message: '请输入角色名称', trigger: 'change' }],
        camerCode: [{ required: true, message: '请选择是否启用', trigger: 'change' }],
        stationCode: [{ required: true, message: '请选择是否启用', trigger: 'change' }]
      },
      dialog: false,
      // 判断状态 添加1 修改2 删除3
      state: '',
  ...
  methods: {
    ...
    initdata () {
      this.$api.product.getCamerList().then(res => {
        this.tableData = res
        // if (res.code === 200) {
    ...
    handleRevise (val, state) {
      this.dialogWidth = '45%'
      this.dialogTitle = '修改监控'
      this.dialog = true
      this.state = state
      let { camerCode, cameraName, stationCode, id } = val
      this.ruleForm = { camerCode, cameraName, stationCode, id }
      // 根据id查数据
      // this.$api.product.getRoleById({ roleId: val.id }).then(res => {
      //   if (res.code === 200) {
