      ...
      <div class="boxContent">
        <el-table :data="tableData"
          ...
          <el-table-column prop="time"
            ...
            label="报警时间"
            align="center"></el-table-column>
          <el-table-column prop="timeExpire"
            :formatter="formatter"
            label="消警时间"
            align="center"></el-table-column>
          <el-table-column prop="totalPrice"
            ...
            <template slot-scope="scope">
              ...
              <div>{{duration(scope.row.time, scope.row.timeExpire)}}</div>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    duration (oldVal, newVal) {
      let oldTime = oldVal * 1000
      let newTime = newVal * 1000
      let timediff = Math.round((newTime - oldTime) / 1000)
      // 获取还剩多少天
    ...
    initdata (val) {
      ...
      if (this.form.applyTime !== null && 0 in this.form.applyTime) {
        starttime = this.form.applyTime[0]
        completetime = this.form.applyTime[1]
      }
      this.$api.product.getHistoryAlarmData({ page: this.page, size: this.limit, townId: this.form.townId, villageId: this.form.villageId, stationId: this.form.stationId, condition: this.form.condition, sensorTypes: this.form.sensorTypes.join(','), startTime: starttime, endTime: completetime }).then(res => {
        if (res.code === 200) {
