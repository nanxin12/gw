...
export default {
  ...
  methods: {
    handleDownload () {
      window.location = 'http://192.168.1.18:8088/material/excel/storage'
    },
    ...
    initdata (val) {
      ...
      this.$api.reportForm.storage({ pageNum: this.page, pageSize: this.limit, starttime, completetime, productName: this.form.productName }).then(res => {
        if (res.code === 200) {
